...
export default function ComercialLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!hasPermission(user, 'campanas:read')) router.replace('/auth/login')
  ...
  const canSeeTraffic = hasPermission(user, 'traffic:read')
  ...
  const currentNav = NAV.find(
    (n) => pathname === n.href || (n.href !== '/comercial' && (pathname ?? '').startsWith(n.href)),
  )
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <aside style={{ width: 220, flexShrink: 0, background: 'var(--bg-surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', padding: '1.25rem 0' }}>
        <div style={{ padding: '0 1.25rem 1.25rem', borderBottom: '1px solid var(--border)', marginBottom: '0.75rem' }}>
          ...
          <div style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--fg)' }}>Comercial</div>
        </div>

        <nav style={{ flex: 1, padding: '0 0.75rem', display: 'flex', flexDirection: 'column', gap: '0.125rem' }}>
          {NAV.map(({ href, label, icon }) => {
            const isActive = pathname === href || (href !== '/comercial' && (pathname ?? '').startsWith(href))
            return (
              <Link
                key={href}
                href={href}

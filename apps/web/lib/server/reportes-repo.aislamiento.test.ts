...
//
// Y NORMALIZA LOS FINALES DE LÍNEA ANTES DE MIRAR, que es lo que este guard
// aprendió al fusionarse con la rama de entidades el 2026-09-18: el archivo se
// escribió con LF y git lo saca con CRLF en cualquier otro árbol (`core.autocrlf`).
// El `.` de JavaScript NO cruza `\r`, así que `//.*$` no llegaba al final de una
// línea CRLF y el comentario NO se quitaba: sobrevivían las propias advertencias
// que citan `qRaw` y el guard se ponía rojo. Rojo en CI y en el árbol de todo el
// mundo menos en el de quien lo escribió, y con el arreglo fácil a la vista:
// borrar la advertencia. Un guard que solo funciona con un final de línea no es
// un guard.

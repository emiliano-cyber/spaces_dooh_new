...
1. ~~**Las e2e no se corrieron.**~~ **HECHO el 2026-09-18, tarde**, con el
   puerto 3311 y `spaces_e2e` ya libres: ver la lista de abajo.
2. ~~**El área `energia` no está registrada**~~ **YA LO ESTÁ**, y lo trajo el
   merge del otro agente de la ola: `lib/modulos.ts:42` y
   `components/demo/shell/nav.ts:130` (medido el 18/09 sobre `main`, commit
   `ac4f71c`). Con eso `moduloDe()` ya resuelve `/energia` y `noAutorizado`
   vuelve a decidir. **Si vuelves a leer este punto en presente, está viejo.**

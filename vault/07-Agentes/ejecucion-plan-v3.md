...
| **P3b-bis · ¿el registro va abierto o cerrado?** | **REDECIDIDA — revierte la del 10/08** | **CERRADO en local y en producción.** `.env.example` baja a `AUTOREGISTRO=0`; el droplet se queda sin la bandera. ⚠️ **Queda abierto si la DEMO de la Fase 4 nace encendida** — F4.4 lo da por hecho: preguntar antes de ejecutarla | 2026-08-14 |
...
> Expediente de evidencia: **[`docs/Instancias_Fase1_Expediente_Cierre.md`](../../docs/Instancias_Fase1_Expediente_Cierre.md)** (commit `fb09b91`).
>
...
| 2026-08-14 | ⚠️ **Lo que esa decisión NO resuelve, y hay que preguntar antes de la Fase 4:** si la **DEMO** nace con el registro encendido. El modelo de instancias soberanas la describe como la única con autoregistro, **F4.4 (`:1345`) lo da por hecho**, y toda la contradicción P4-bis se resolvió precisamente para que DEMO pudiera tenerlo abierto sin duplicar imágenes. «Producción» hoy **es** `demo.space-os.io`, así que las dos lecturas son defendibles. **No se asume ninguna.** |
...
| 2026-08-14 | **FASE 1 CERRADA EN LOCAL.** Expediente commiteado en `docs/Instancias_Fase1_Expediente_Cierre.md` (`fb09b91`, 17 secciones). Es la primera vez que corre el `documentalista`, escrito hoy a raíz de la regla de cierre de fase. **La fase no se declaró cerrada hasta que el expediente existió**, que es lo que esa regla exige. |

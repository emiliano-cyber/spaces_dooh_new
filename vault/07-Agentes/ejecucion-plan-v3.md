...
| F2.5 | [verificación] | ensayista-local | F2.2 | **ENSAYADA_LOCAL** | Smoke en verde: `200 · 200 · 503 · 401`, y **el login con estilos** (18 activos a 200, el CSS con 707 reglas). ⚠️ Dos hallazgos que el plan no anticipa: **la imagen no puede levantar una base nueva sola** (falta el rol de app) y **el 503 sale por el motivo contrario al que dice el plan** — ver bitácora |
| F2.6 | [código] | ejecutor | F2.1 | **DESBLOQUEADA — obligatoria, y con el alcance cambiado** | El ensayo de F2.5 midió que **la mitad de servidor YA se comporta como runtime** (misma imagen: `=0`→503, `=1`→400). Lo que falta es el **cliente**: `login/page.tsx:30` se prerrenderiza en el build, así que el valor **debe llegar por `GET /api/auth/metodos/`** — el `[SIN VERIFICAR]` del paso 3 queda resuelto por evidencia. ⚠️ Sigue invirtiendo la polaridad por omisión |

...
| F3.7 | [infra] | ejecutor escribe script; ensayo parcial (destino local en vez de Spaces) | F3.4 ✅ | ✅ **COMPLETADA_LOCAL** — `f369b4c`, AMARILLO, **en auditoría** | Nace `infra/scripts/respaldo.sh` (258 líneas) y el paso 3 de `update.sh` hace tres cosas: dump, poda local y subida. Arnés de **37·165 con 10 mutantes** a **48 escenarios · 218 comprobaciones · 0 rojas** con **17 mutantes, 0 escapan** (~45 min). ▸ **Su ejecutor encontró y corrigió un defecto propio antes de cerrar, y volvió a medir entero**: la poda corría con el `set -e` de `update.sh` **activo dentro de la función**, así que un `rm` que fallara —directorio de solo lectura, bit inmutable— habría **matado el update entre el respaldo y las migraciones**, con un código a secas y sin una línea que lo explicara. Ahora va en `if !` y su fallo es un AVISO. Tiró una corrida de mutantes de 15 minutos y la repitió sobre el código corregido. ▸ Credenciales **solo documentadas** —ninguna entra al repo— y **no viajan en `argv`**: archivo temporal con `chmod 600` pedido ANTES de escribir el secreto. ▸ **Fail-closed**: sin `respaldo.sh` al lado, el update se para ANTES del `pull`. La subida real a Spaces es tarjeta humana (TH-F37-A a D). **Cierra la segunda mitad de D4**: la retención de 3 respaldos locales que pide su paso 4 es exactamente la poda de `DIR_RESPALDOS` que hoy no existe. ⚠️ La retención remota (30 días) va por **regla de ciclo de vida del bucket, NO por un `rm` en el script** — el plan explica por qué y no es una preferencia de estilo |
...
>
> - **H1** · `update.sh:863` y `:873` —desde `f369b4c`; fueron `:816`/`:826` en `84c6c20` y la auditoría las citó como `:755` y `:765`, antes de que el archivo creciera de 786 a 847 líneas— la frase «el contenedor de la version anterior esta

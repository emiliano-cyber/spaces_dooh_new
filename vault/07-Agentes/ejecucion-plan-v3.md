...
| **T-01** | [código] · **fuera del plan** | ejecutor | — | EN_CURSO (2º intento) | `bootstrap-auth.mjs` está roto por **dos** causas, no una. Alcance ampliado por Jochelo el 13/08: se arreglan las dos en el mismo commit. **Su contrato es el prompt, no el plan** |
| F1.2 | [migración] | ejecutor | F1.1-local + **T-01** | **BLOQUEADA** → por T-01 | Descubre tablas por catálogo, no por lista; ensayo de idempotencia ×2 en `spaces_e2e`. ⚠️ El plan manda reverificar el `insert into reservas` que cita en `campanas-repo.ts:687-696`: tras F1.3 arranca en **`:697`** |

...
>
> *(Esta cuenta subió a TRES a mitad del 02/09, al descubrirse que F2.4 estaba
> cerrada en falso, y volvió a DOS esa misma tarde al cerrarla de verdad. Las dos
> vueltas quedan escritas en los callouts de abajo: la cuenta no bajó por
> reinterpretar nada, bajó porque se corrió el workflow.)*
>
...
> [!success] 2026-09-02, tarde · **F2.4 CERRADA de verdad — la FASE 2 queda completa**
> `v0.3.0` en **`estable`**. Primera versión que llega al canal que consumen las
> instancias: la del 01/09 nunca llegó.
>
> **Lo que lo hace un cierre:** el paso «Comprobar que promover no cambio el
> digest» releyó el registro después de reetiquetar y coincidió con el validado.
> Ese paso **es** el criterio de aceptación de la tarea, y pasó en verde por
> primera vez desde que la tarea existe.
>
> **Cuatro intentos, y ninguno por el código de la aplicación:**
>
> | Intento | Murió en | Por qué |
> |---|---|---|
> | 1.º | 1.ª puerta | `DEMO_URL` pegada con un espacio delante |
> | 2.º | puerta 3 | corrió el workflow **viejo**: `workflow_dispatch` toma el archivo de la rama elegida, y se lanzó antes de fusionar el arreglo |
> | 3.º | 2.ª puerta | la versión pegada con un punto detrás: `'v0.3.0.'` |
> | 4.º | — | verde |
>
> Los tres primeros los cazó un guard **imprimiendo el valor entre comillas**, que
> es lo único que hace visible un espacio o un punto, y ninguno llegó a tocar el
> registro. Vale como argumento a favor de esa costumbre: un mensaje que dice
> «valor no válido» sin enseñarlo habría costado la tarde.

> [!danger] 2026-09-02, mañana · **F2.4 estaba cerrada EN FALSO. Lo de abajo es FALSO**
...
|---|---|---|---|
| **P5 · qué dirección representa a DEMO** para el smoke de `promover.yml` | ✅ **RESUELTA el 2026-08-31: `prueba.space-os.io`** — nombre NUEVO, no `demo.space-os.io`, que quedó cerrado como la demo original que se elimina. Y **resultó mucho más barata de lo que se creía**: `promover.yml:241-290` solo exige dos códigos 200 sobre `https`, así que **F2.4 no necesita contenerizar DEMO** — eso es F3.5, que es otra cosa. ⚠️ **Mientras DEMO no corra desde la imagen, `FLOTA_TOKEN` no debe existir como secreto de GitHub**: `api/version/route.ts:102` devolvería `'desconocida'`, que no es vacío, y el smoke fallaría siempre. Censo y medición en `docs/evidencias/censo-demo-resultado-20260831.md`. *(Antes decía: APLAZADA CON DISPARADOR antes de F5.6)* ⚠️ *(era F5.7; corregido el mismo dia por la auditoria de la Fase 5)* | **No se decide hoy, y no bloquea nada de lo que hay por delante.** Solo detiene **F2.4** —marcar una versión como `estable`—. ⚠️ **Corrección del 31/08, tarde**: se dijo que `estable` no lo necesitaba nadie hasta F5.7, y es **falso**. Lo necesita ya **F5.6**, el ensayo en un droplet desechable: el alta instala `estable` y `update.sh:768-770` **se para en seco** si esa etiqueta no existe. La salida alternativa —ensayar con `CANAL=beta`— existe y la máquina la admite, pero es una desviación del runbook que se está ensayando y **la decide una persona**. Evidencia en `docs/evidencias/auditoria-f5-31-agosto.md`. ▸ **Y hay que desdoblar el nombre, porque es la confusión que costó una vuelta entera el 31/08**: `demo.space-os.io` es el **droplet viejo**, que sale del modelo y se elimina (ADR 0023/0024) — eso está cerrado y no se vuelve a preguntar. La **DEMO del modelo** es otra cosa: el proceso del **3001 dentro del PADRE**, que vive y funciona pero **no tiene nombre público**. ▸ **Lo que NO es DEMO**: el plano de control —ver la flota y dar de alta instancias— es el **3000**, el PADRE, en `space-os.io`, y ya está construido (Fase 6). ▸ **Cuando toque, la salida es dar un nombre NUEVO al 3001** con su certificado; `promover.yml:127-129` exige `https://`, así que un `127.0.0.1:3001` no vale. Reutilizar `demo.space-os.io` está descartado | 2026-08-31 |
...
| **P4 · nombre del registry** | **RESUELTA** | **DigitalOcean Container Registry.** Login con `secrets.DO_REGISTRY_TOKEN`. **No cambia una línea de F2.3/F2.4**: el plan (`:785-786`) ya manda elegir el login según `vars.REGISTRY_TIPO` para no reescribir el workflow cuando cayera esta decisión. Lo que fija son **dos variables de repositorio en GitHub** → tarjeta **TH-P4**. ⚠️ **Arrastra P3**: un registry de DO vive en una cuenta concreta, y en cuál nacen las instancias sigue abierto. Mirar además el límite de almacenamiento del plan contratado. ✅ **CERRADA DEL TODO el 2026-08-31**: el registro existe, se llama **`registryspaces`** (`registry.digitalocean.com/registryspaces`), región **NYC3** y **plan gratuito — 500 MiB, un repositorio**. El límite de un repositorio no estorba: `release.yml:268-269` publica uno solo, `$REGISTRY/space-os`, y `beta`/`estable` son **etiquetas** sobre la misma imagen. **Medido el 2026-08-31 con `v0.0.1-rc2` dentro: 11 % de 500 MiB, ~55 MiB.** El plan gratuito sobra; falta el coste incremental de la segunda versión, que es el que dice cuántas caben. Tarjeta nueva: `docs/evidencias/registry-TH-P4b.txt`, que **sustituye a TH-P4**, cuyo bloque de la etiqueta mandaba a un worktree que ya no existe | 2026-08-17 · cerrada 2026-08-31 |

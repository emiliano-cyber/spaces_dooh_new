...
| Z10 · UI base 🟡 | LIBRE | — | — | — | — | **13/08 · `font-mono` NO es monoespaciada**, es Inter con `tnum`: es la clase de los NÚMEROS. Para código hay token propio, `--font-codigo` → clase `font-codigo` (`ui-monospace`, sin descarga), y sus únicos tres consumidores son el HTML de los creativos (`creativos/page.tsx:598,874` y `AgregarCreativo.tsx:106`). — **13/08 · Tipografía institucional.** Titulares en **Source Serif 4** (`--font-display`), todo lo demás e **todos los números** en **Inter** (`--font-sans`), con cifras tabulares en `.demo-num` y en la clave `font-mono` de Tailwind — que ya **no** es monoespaciada. Las dos por `next/font` en `app/layout.tsx`: cero peticiones a un CDN en ejecución. Se cambian aquí y en `demo.css`, **nunca por componente**: los nombres de clases y variables no se tocaron. — `Button` se bloquea solo mientras su `onClick` esté en vuelo (10/08, A5). No cambia su API y ningún formulario se tocó. La guarda vive en `lib/clic-unico.ts` y tiene sus propias pruebas. **11/08: el menú va por fases del proceso** (`nav.ts`, campo `grupo` + `GRUPOS`); si añades un módulo, ponle grupo o `nav.test.ts` se pone roja |
| Z11 · Utilidades 🟢 | LIBRE | — | — | — | — | Zona de entrada para agentes nuevos |
| Z12 · Docs 🟢 | LIBRE | — | — | — | — | Bóveda creada el 07/08, **validada contra el código el 10/08** y actualizada con la tarde del 10/08 ([[2026-08-10]]). **25/08: manual de usuario reescrito de cero** contra la interfaz — [[manual-usuario-2026-08-25]] sustituye al borrador del 11/08, que queda marcado `estado: superado`. Cubre las 18 áreas del menú, las tres pantallas públicas, el diccionario de estados y el capítulo de fallas con los mensajes literales del servidor. Si tocas rótulos de la UI, ese manual los cita: hay que actualizarlo en el mismo commit |
...
| `apps/web/app/layout.tsx` | LIBRE | — | — |
| `apps/web/tailwind.config.ts` | LIBRE | — | — |

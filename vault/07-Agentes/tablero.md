...
| Z10 · UI base 🟡 | LIBRE | — | — | — | — | **13/08 · Tipografía institucional.** Titulares en **Source Serif 4** (`--font-display`), todo lo demás e **todos los números** en **Inter** (`--font-sans`), con cifras tabulares en `.demo-num` y en la clave `font-mono` de Tailwind — que ya **no** es monoespaciada. Las dos por `next/font` en `app/layout.tsx`: cero peticiones a un CDN en ejecución. Se cambian aquí y en `demo.css`, **nunca por componente**: los nombres de clases y variables no se tocaron. — `Button` se bloquea solo mientras su `onClick` esté en vuelo (10/08, A5). No cambia su API y ningún formulario se tocó. La guarda vive en `lib/clic-unico.ts` y tiene sus propias pruebas. **11/08: el menú va por fases del proceso** (`nav.ts`, campo `grupo` + `GRUPOS`); si añades un módulo, ponle grupo o `nav.test.ts` se pone roja |

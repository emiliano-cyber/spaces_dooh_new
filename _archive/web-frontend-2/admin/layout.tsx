...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!ADMIN_ROLES.has(user.rol)) router.replace('/comercial')
  ...
  if (!ADMIN_ROLES.has(user.rol)) return null
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <aside style={{ width: 220, flexShrink: 0, background: 'var(--bg-surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', padding: '1.25rem 0' }}>
        <div style={{ padding: '0 1.25rem 1.25rem', borderBottom: '1px solid var(--border)', marginBottom: '0.75rem' }}>
          ...
          <div style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--fg)' }}>Administración</div>
        </div>
        ...
        <nav style={{ flex: 1, padding: '0 0.75rem', display: 'flex', flexDirection: 'column', gap: '0.125rem' }}>
          {NAV_PRIMARY.map(({ href, label, icon, exact }) => {
            ...
            return (
              <Link key={href} href={href} style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', padding: '0.5rem 0.75rem', borderRadius: '6px', fontSize: '0.875rem', fontWeight: isActive ? 600 : 400, color: isActive ? 'var(--fg)' : 'var(--muted)', background: isActive ? 'var(--bg-hover)' : 'transparent', transition: 'all 0.15s', textDecoration: 'none' }}>
          ...
          <div style={{ marginTop: '1.25rem', paddingTop: '1rem', borderTop: '1px solid var(--border)' }}>
            <div style={{ fontSize: '0.65rem', fontWeight: 600, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.08em', padding: '0 0.75rem', marginBottom: '0.375rem' }}>
              Módulos
            </div>
            {NAV_MODULES.map(({ href, label }) => (
              <Link key={href} href={href} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.375rem 0.75rem', borderRadius: '6px', fontSize: '0.8rem', color: 'var(--muted)', textDecoration: 'none', transition: 'color 0.15s' }}>
        ...
        <div style={{ padding: '1rem 1.25rem', borderTop: '1px solid var(--border)' }}>
          <div style={{ fontSize: '0.75rem', color: 'var(--muted)', marginBottom: '0.375rem' }}>{user.rol}</div>
          <button onClick={logout} style={{ background: 'none', border: '1px solid var(--border)', borderRadius: '6px', color: 'var(--muted)', cursor: 'pointer', fontSize: '0.8125rem', padding: '0.375rem 0.75rem', width: '100%', transition: 'all 0.15s' }}>
            Salir
          </button>
        </div>
      </aside>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <header style={{ height: 52, borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', padding: '0 1.5rem', justifyContent: 'space-between', background: 'var(--bg-surface)', flexShrink: 0 }}>
          <span style={{ fontSize: '0.875rem', fontWeight: 500, color: 'var(--fg)' }}>
            {NAV_PRIMARY.find((n) => n.exact ? pathname === n.href : (pathname ?? '').startsWith(n.href))?.label ?? 'Admin'}
          </span>

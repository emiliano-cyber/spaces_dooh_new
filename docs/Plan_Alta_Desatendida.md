...
## Fase 0 · Lo que se puede hacer ya, sin riesgo y sin arnés
...
### A0.1 · Las comprobaciones van dentro de la solicitud `[código]`

- **Objetivo:** que el resultado de un alta sea auditable tres semanas después, y no solo
  mientras el `journalctl` lo conserve.
- **Archivos:** `apps/flota/altas.mjs`, `apps/flota/cola.mjs`, `apps/flota/altas.test.ts` (nuevo o el que haya).
- **Qué hace:** al terminar un alta, ejecutar las tres comprobaciones del paso 5 de
  `TH-ALTA_despues-del-formulario.txt` —`login`, `signup`, `login-post`— y guardarlas en la
  solicitud, en un campo `comprobaciones`.
- **Prueba que falla primero:** dos casos, y **el negativo es el que importa** — una
  comprobación que devuelve un código inesperado tiene que quedar **anotada como tal**, no
  descartada ni convertida en un fallo del alta.
- **Criterio:** una solicitud terminada trae los tres códigos; una con un código raro los
  trae igual y el alta no se marca `fallida` por ello.
- **Riesgo:** ninguno para la instancia. Son tres `curl` de lectura.
- **Commit:** `feat(altas): las comprobaciones del alta quedan en la solicitud`

### A0.2 · El token de flota se entrega por archivo `[código + infra]`

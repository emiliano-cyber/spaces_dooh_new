...
| 2026-08-06 | `20260806_m13b_razon_social_g500.sql` | Hallazgo **M13b**: la razón social de `g500` seguía siendo `DEMO RGB CATORCE S DE RL DE CV` — quedó fuera del A9 a propósito, para resolverse junto con M5. Recorta el prefijo y deja `RGB CATORCE S DE RL DE CV`, que es la razón social **legal** de la misma organización cuyo nombre comercial es G500 (confirmado el 06/08). Importa porque `tenants.razon_social` es la parte arrendataria del contrato que se manda a firma. 1 fila. Rollback: `20260806_m13b_razon_social_g500_rollback.sql`. |

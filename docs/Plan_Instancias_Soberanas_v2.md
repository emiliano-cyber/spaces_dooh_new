# Plan de implementación v2 · Modelo de despliegue por instancias soberanas
...
> **Sustituye a:** `2026-08-11-subdominios-por-tenant-plan.md` (archivado).
...
> `dfccd04`. Todas las rutas y líneas se abrieron con Read/Grep el 2026-08-12.
> **No se ejecutó nada:** ni `ssh`, ni `curl` a producción, ni `doctl`, ni
> escritura en el repo. Los comandos contra servidores están escritos para que los
> corra una persona.
...
| **2** · Release versionado | F2.1, F2.2, F2.3, F2.4, F2.5 | Completa **con el registry como parámetro**. Bloqueado solo el valor → §8.4 |
| **3** · `update.sh` + runner | F3.1, F3.2, F3.3, F3.4, F3.5, F3.6 | Completa. F3.5 depende de la Fase 4 (P5). F3.6 es un añadido obligado por el invariante 2 |
| **4** · Separar DEMO | F4.1, F4.2, F4.3, F4.4, F4.5 | Completa |
| **5** · `provision-instancia.sh` | F5.1, F5.2, F5.3, F5.4, F5.5, F5.6, **F5.7 bloqueada** | Completa salvo el alta real → §8.2 y §8.3 |
| **6** · Visibilidad de flota | F6.1, F6.2, F6.3 | Completa |
...
**Total: 40 tareas.** 33 ejecutables hoy; 3 bloqueadas de raíz (F5.7, F7.2, F7.3); 2
bloqueadas solo en un valor (F2.3, F2.4 esperan el nombre del registry); 3 con
dependencia de orden (F3.5, F3.6 y F6.3 esperan a la Fase 4).
...
| | `version.e2e.test.ts` (F6.1) | 4 |
| **Subtotal integración** | | **19** |
| **Total** | | **35** |

De esos 35, **14 son negativos** (el insert que debe fallar, el tenant huérfano que no
...
- No tocó ni un archivo del repo. El árbol del worktree quedó igual que al empezar.
- No decidió ninguna de las cuatro de §8.
